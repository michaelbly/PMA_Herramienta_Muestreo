...
# It includes functions for creating a sampling frame, performing Muestreo por conglomerados, random sampling, and two-stage random sampling.
...
# If the sampling method is "Estratificado", the strata_id column is created by extracting the values from the specified strata column in the input data frame.
# If the sampling method is "Muestreo por conglomerados", the psu_id column is created by extracting the values from the specified col_psu column in the input data frame,
# and the pop_numbers column is created by extracting the values from the specified colpop column in the input data frame.
# If the sampling method is "Asignacion aleatoria simple" or any other method, the psu_id column is created using the id_sampl column,
...
# If the sampling method is "Muestreo por conglomerados", the psu_id column is converted to a factor.
...
# clustersample function performs Muestreo por conglomerados based on the given parameters.
# Parameters:
...
#' Perform Muestreo por conglomerados
#'
#' This function performs Muestreo por conglomerados based on specified parameters.
#' sframe A data frame containing the sampling frame data.
...
#' The sampling methods supported are Muestreo por conglomerados, Asignacion aleatoria simple, and Muestreo aleatorio sampling.
...
make_sample<-function(sampling_frame,input){
  ...
  } else if (input$samp_type=="Muestreo aleatorio"){
  ...
  if(input$samp_type=="Muestreo por conglomerados"){
    for(i in 1:nrow(summary_sample)){
      if(summary_sample$strata_id[i]%in%sw_rand){
        ...
        summary_sample$Sampling_type[i]<-"Muestreo por conglomerados with size 1 = random sampling"
      }
    }
  }
